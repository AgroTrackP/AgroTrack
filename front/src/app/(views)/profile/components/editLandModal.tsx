...
const EditLandModal: React.FC<EditLandModalProps> = ({ isOpen, onClose, onConfirm, landData }) => {
    const [formData, setFormData] = useState<LandFormData>({
        ...landData,
        area_m2: String(landData.area_m2 || ''),
        start_date: landData.start_date ? new Date(landData.start_date).toISOString().split('T')[0] : ''
    });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                <h2 className="text-xl font-bold mb-6 text-gray-800">✏️ Modificar Terreno</h2>
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* Tipo de Cultivo */}
                        <div>
                            <label htmlFor="crop_type" className="block text-sm font-medium text-gray-700">Tipo de Cultivo</label>
                            <select
                                name="crop_type"
                                id="crop_type"
